...
class Admin:
    def __init__(self):
        # Créer la fenêtre
        self.window = Tk()
        self.window.title("PAGE PEDAGOGIE")
        self.window.geometry("1350x750+0+0")
        self.window.resizable(False, False)
        self.window.config(bg="#091821")

        # Ajouter un titre
        h2 = Label(self.window, borderwidth=3, relief=SUNKEN, text="GESTION DES ETUDIANTS ", font=("sans serif", 25), bg="#2F4F4F", fg="#FFFAFA")
        h2.place(x=0, y=0, width=1350, height=100)

        # Ajouter un titre pour le formulaire
        h1 = Label(self.window, text="formulaire d'inscription des etudiants", font=("helvetica", 18), bg="#091821", fg="#FFFAFA")
        h1.place(x=50, y=150, width=450)

        # Ajouter un champ pour le nom
        lbnom = Label(self.window, text="NOM :", font=("Arial", 18), bg="#091821", fg="white")
        ...
        lbp = Label(self.window, text="PRENOM :", font=("Arial", 18), bg="#091821", fg="white")
        ...
        email = Label(self.window, text="E-MAIL :", font=("Arial", 18), bg="#091821", fg="white")
        ...
        # Ajouter une liste déroulante pour la formation
        lbMatiere = Label(self.window, text="FILIERE :", font=("Arial", 18), bg="#091821", fg="white")
        lbMatiere.place(x=70, y=400, width=130)
        self.matiere1 = ttk.Combobox(self.window, font=("Arial", 16))
        self.matiere1['value'] = ['BTS SIO1', 'BTS SIO2']
        self.matiere1.place(x=250, y=400, width=210)

        # Ajouter un champ pour l'option
        lbNote = Label(self.window, text="OPTION :", font=("Arial", 18), bg="#091821", fg="white")
        lbNote.place(x=70, y=450, width=150)
        self.ChampNote = ttk.Combobox(self.window, font=("Arial", 16))
        self.ChampNote['value'] = ['SLAM','SISR']
        self.ChampNote.place(x=250, y=450, width=130)

        # Ajouter un champ pour la date d'inscription
        types = Label(self.window, text="DATE:", font=("Arial", 18), bg="#091821", fg="white")
        ...
        #Enregistrer
        btnE = Button(self.window, text="Enregistrer", font=("Arial", 16), bg="#D2691E", fg="white", command=self.ajouter)
        btnE.place(x=250, y=600, width=200)


        #voir list
        btnS = Button(self.window, text="Modifier", font=("Arial", 16), bg="#D2691E", fg="white", command=self.modifier)
        btnS.place(x=250, y=650, width=200)

        #Enregistrer
        btnE = Button(self.window, text="Supprimer", font=("Arial", 16), bg="#D2691E", fg="white", command=self.supprimer)
        ...
        self.tree.heading(5, text="FILIERE")
        self.tree.heading(6, text="OPTION")
        ...
        con = MC.connect(host="localhost", user="root", password="", database="elite_db")
        cur = con.cursor()
    ...
    def ajouter(self):
        ...
        if self.Champnom.get() == "" or self.ChampP.get() == "" or self.champmail.get() == "" or self.matiere1.get() == "" or self.ChampNote.get() == "" or self.type_user.get() == "":
            ...
            cur.execute("SELECT * FROM inscription WHERE nom = %s AND prenom = %s AND mail = %s", (self.Champnom.get(), self.ChampP.get(), self.champmail.get()))
            row = cur.fetchone()
            if row:
                messagebox.showerror("", "Cet étudiant existe déjà dans la base de données.")
                self.vider()
            else:
                # Insérer les valeurs dans la base de données
                sql = "INSERT INTO inscription (nom, prenom, mail, filiere, option, date) VALUES (%s, %s, %s, %s, %s, %s)"
                val = (self.Champnom.get(), self.ChampP.get(), self.champmail.get(), self.matiere1.get(), self.ChampNote.get(), self.type_user.get())
                cur.execute(sql, val)
                con.commit()
                messagebox.showinfo("Information", "Etudiant enregistré.")
                self.vider()
                self.window.destroy()
    ...
    def modifier(self):
        ...
        con = MC.connect(host="localhost", user="root", password="", database="elite_db")
        cur = con.cursor()
        cur.execute("UPDATE inscription SET nom=%s, prenom=%s,mail=%s ,filiere=%s, option=%s ,date=%s  WHERE id=%s",
